...
export async function connectSalesforce(instanceUrl, clientId) {
  ...
  // Salesforce identity URL format: https://login.salesforce.com/id/{orgId}/{userId}
  const idParts  = (tokenData.id || '').split('/');
  const sfUserId = idParts.length >= 2 ? idParts[idParts.length - 1] || null : null;
  const sfOrgId  = idParts.length >= 2 ? idParts[idParts.length - 2] || null : null;
  ...
  return { userId: sfUserId, orgId: sfOrgId };
}
...
async function pollDeployResult(instanceUrl, apiVersion, jobId, onProgress) {
  for (let attempt = 1; attempt <= MAX_DEPLOY_POLL_ATTEMPTS; attempt++) {
    // Wait before polling (skip the very first attempt to catch fast deploys quickly)
    if (attempt > 1) {
...
export async function deployFlow(flowXml, apiVersion, onProgress = () => {}) {
  ...
  // Sanitise: replace spaces/hyphens with underscores, strip any other invalid chars
  const sanitised = rawName.replace(/[\s-]+/g, '_').replace(/[^a-zA-Z0-9_]/g, '');

  // Salesforce API names must start with a letter; prepend 'Flow_' if needed
  let flowName;
  if (!sanitised) {
    flowName = `GeneratedFlow_${Date.now()}`;
  } else if (/^[^a-zA-Z]/.test(sanitised)) {
    flowName = `Flow_${sanitised}`;
  } else {
    flowName = sanitised;
  }
